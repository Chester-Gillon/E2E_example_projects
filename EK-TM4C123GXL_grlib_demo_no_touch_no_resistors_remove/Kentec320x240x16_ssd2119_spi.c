...
InitSPILCDInterface(uint32_t ui32SysClock)
{
    ...
    /* @todo Increase the drive strength from the default 2mA to 8mA to compensate for the 2nF to ground caused when
     *       connecting the BOOSTXL-K350QVG-S1 but *not* removing R9 and R10 on the EK-TM4C123GXL.
     */
    GPIOPadConfigSet(LCD_SSI_GPIO_BASE, LCD_SSI_TX_PIN, GPIO_STRENGTH_8MA, GPIO_PIN_TYPE_STD);
    ...
    //
    /* @todo SSI frequency reduced from 10 MHz to 1 MHz for the same reason as increasing the drive strength */
    SSIConfigSetExpClk(LCD_SSI_BASE, ui32SysClock, SSI_FRF_MOTO_MODE_0,
            SSI_MODE_MASTER, 1000000/*10000000*/, 8);
